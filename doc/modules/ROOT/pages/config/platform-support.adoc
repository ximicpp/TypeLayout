...
// Verify type is safe for cross-platform binary serialization
static_assert(is_serializable_v<NetworkPacket, PlatformSet::bits64_le()>, 
    "NetworkPacket must be serializable for network transmission");
----
...
// Windows LLP64 model
sizeof(int)       == 4   // �?
sizeof(long)      == 4   // �?(different from Linux!)
sizeof(long long) == 8   // �?
sizeof(size_t)    == 8   // �?
...
// Linux LP64 model
sizeof(int)       == 4   // �?
sizeof(long)      == 8   // �?(different from Windows!)
sizeof(long long) == 8   // �?
sizeof(size_t)    == 8   // �?
...
=== 2. Verify Serializability at Compile Time

[source,cpp]
----
template<Serializable T>
void send_over_network(const T& data) {
    // The Serializable concept ensures the type is safe
    // for binary transmission across platforms
    // ...
}
----
...
* xref:../user-guide/portability.adoc[Serializability Guide]
