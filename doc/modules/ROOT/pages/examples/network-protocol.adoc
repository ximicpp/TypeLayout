...
== The Challenge: Cross-Machine Data Exchange

[IMPORTANT]
====
This example demonstrates **🌐 Cross-Machine** data sharing—one of the three boundaries where TypeLayout's core guarantee applies:

**Same Signature ⟺ Same Memory Layout ⟺ Safe to transmit raw bytes**

When two machines exchange binary data over the network, they MUST agree on the exact byte layout. Different compilers, platforms, or library versions can silently break this. TypeLayout catches these issues at compile time.
====
...
// Verify header is serializable at compile time
static_assert(Serializable<MessageHeader>,
    "MessageHeader must be serializable for network transmission");
...
static_assert(Serializable<AuthRequest>);
...
static_assert(Serializable<AuthResponse>);
...
static_assert(Serializable<DataMessage>);
...
static_assert(Serializable<Handshake>);
