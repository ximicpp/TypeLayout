...
xref:network-protocol.adoc[Network Protocol]::
Using TypeLayout for binary protocol validation.

xref:shared-memory.adoc[Shared Memory]::
...
struct NonPortableConfig {
    long value;  // Size varies by platform
};

static_assert(is_portable<PortableConfig>());    // ✓ Passes
// static_assert(is_portable<NonPortableConfig>()); // ✗ Fails
----
...
void send_to_network(const T& data) {
    // Only accepts types with portable layout
