...
static_assert(is_portable<SharedHeader>(),
    "SharedHeader must be portable across processes");
...
static_assert(is_portable<SharedData>(),
    "SharedData must be portable across processes");
...
struct SharedData {
    std::atomic<std::uint64_t> counter;  // ✓
    std::uint64_t unsafe_counter;        // ✗ Race condition!
};
----
...
* xref:serialization.adoc[Serialization Example]
