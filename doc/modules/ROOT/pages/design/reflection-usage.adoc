...
=== Pattern 2: Serializability Check

[source,cpp]
----
template<typename T, PlatformSet P>
constexpr bool check_serializable_members() {
    template for (constexpr auto member : nonstatic_data_members_of(^T)) {
        // Reject bit-fields for serialization
        ...
        // Check if this member is serializable on the target platform
        if (!is_serializable_v<MemberType, P>) {
            return false;
        }
    }
    return true;
