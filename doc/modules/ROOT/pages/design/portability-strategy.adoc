...
For compound types, we check all members recursively using `is_serializable_v`:

[source,cpp]
----
// Platform set defines target architecture
constexpr auto platform = PlatformSet::bits64_le();

// Check if type is serializable for the target platform
static_assert(is_serializable_v<MyData, platform>);
----
...
serialize(Good{});  // OK
serialize(Bad{});   // Compile error: long has platform-dependent size
----
...
static_assert(!boost::typelayout::is_serializable_v<PlatformConfig, PlatformSet::current()>);
----
...
----
struct MixedData {
    std::int32_t portable_id;    // Serializable
    long platform_handle;         // Non-serializable (platform-dependent)
};

// Entire struct is non-serializable due to one member
static_assert(!boost::typelayout::Serializable<MixedData>);
----
