...
Constrains a type to be safe for binary serialization (no platform-dependent, pointer, or bit-field members).
...
A type `T` satisfies `Serializable` if and only if `is_serializable_v<T, PlatformSet::current()>` is `true`.

A type is serializable when it contains no:

* Platform-dependent types (`long`, `unsigned long`, `wchar_t`, `long double`)
* Pointers (size varies between 32-bit and 64-bit)
...
SerializableData sd;
write_to_network(sd);  // OK

NonSerializableData nsd;
// write_to_network(nsd);  // Error: constraint not satisfied
----

=== See Also

* xref:reference/core-functions.adoc#is_serializable_v[is_serializable_v] - Underlying variable template
