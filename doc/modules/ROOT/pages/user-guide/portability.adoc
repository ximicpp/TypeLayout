...
Use `is_serializable_v<T, P>` to check if a type is safe for binary serialization on a target platform set:
...
// Compile-time checks using the current platform
constexpr auto platform = PlatformSet::current();

static_assert(is_serializable_v<SerializableData, platform>);
static_assert(!is_serializable_v<NonSerializableData, platform>);
----

== The Serializable Concept

Use the `Serializable` concept to constrain templates:
...
void serialize_to_network(const T& data, std::span<std::byte> buffer) {
    // Only works with types safe for binary serialization
...
struct BadPacket {
    unsigned long sequence;  // Non-serializable: platform-dependent size!
    std::uint16_t length;
};
...
== Serializability Checking for Nested Types

Serializability checking is recursive - a struct is only serializable if all its members are serializable:
...
struct Outer {
    std::int32_t x;
    Inner inner;  // Contains non-serializable member
};

static_assert(!boost::typelayout::Serializable<Inner>);
static_assert(!boost::typelayout::Serializable<Outer>);  // Also non-serializable
----

== Pointers and Serializability

Pointers are always platform-dependent (32-bit vs 64-bit), so any struct containing pointers is non-serializable:
...
static_assert(!boost::typelayout::Serializable<WithPointer>);
----

If you need serializable data structures with references to other data, consider using indices or offsets instead:

[source,cpp]
----
struct SerializableWithReference {
    std::uint32_t data_offset;  // Offset instead of pointer
    std::uint32_t data_count;
};

static_assert(boost::typelayout::Serializable<SerializableWithReference>);
----
...
* xref:user-guide/type-support.adoc[Type Support] - Complete type list
* xref:user-guide/hash-verification.adoc[Hash Verification] - Runtime verification
* xref:user-guide/concepts.adoc[Concepts] - Using the `Serializable` concept
