...
TypeLayout fully supports both `struct` and `class` types, including classes with:

* **Private/protected members** - ALL members are reflected regardless of access specifier
* **Constructors/destructors** - Non-trivial classes are fully supported
* **Inheritance** - Single, multiple, and virtual inheritance
* **Virtual functions** - Polymorphic classes include vtable in layout
* **Static members** - Correctly excluded from signatures
...
TypeLayout reflects ALL members, including private ones:
...
private:
    uint64_t id_;    // TypeLayout reflects this!
    bool active_;    // TypeLayout reflects this!
};

static_assert(LayoutSupported<Entity>);  // Classes are fully supported
----
...
TIP: TypeLayout is *NOT* limited to POD types or structs. It supports the full spectrum of C++ types.
...
[source]
----
[64-le]class[s:8,a:4,inherited]{@0[base]:struct[s:4,a:4]{@0[baseValue]:i32[s:4,a:4]},@4[derivedValue]:i32[s:4,a:4]}
----

NOTE: The `inherited` marker indicates the class has base classes.
...
[source]
----
[64-le]class[s:16,a:8,polymorphic]{@8[id_]:i32[s:4,a:4]}
----

NOTE: The `polymorphic` marker indicates the class has a vtable. The vtable pointer occupies bytes 0-7, so member `id_` starts at offset 8.
