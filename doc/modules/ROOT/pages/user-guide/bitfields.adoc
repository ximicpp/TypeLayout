...
// Types with bit-fields are NOT serializable (bit packing is platform-specific)
static_assert(!boost::typelayout::is_serializable_v<Flags, PlatformSet::current()>);

// The serialization_status indicates the reason
// Returns: "!serial:bitfield"
----

IMPORTANT: Types containing bit-fields are **always non-serializable** in Layer 2 audits, because bit-field packing order and storage unit rules vary across platforms and compilers. Use `TYPELAYOUT_BIND` only to verify the **layout signature** (Layer 1), not serializability.
...
* xref:user-guide/portability.adoc[Serializability] - Serialization considerations
