...
== Padding Detection and Implicit Capture

=== How Padding Appears in Signatures
...
=== Why Signatures Implicitly Capture Padding
...
3. **Member sizes** — captured in each field's type signature

[IMPORTANT]
====
**Mathematical Proof**: Given `sizeof(T)` and all member offsets/sizes, padding is uniquely determined.

If ANY padding changes:

* Either `sizeof(T)` changes → signature differs at `[s:N,...]`
* Or a member offset changes → signature differs at `@N[...]`

There is no way to change padding without changing the signature.
====
...
// Explicit padding
struct B { char c; char pad[3]; int i; };
----
...
Then their memory layouts ARE identical, and TypeLayout correctly produces the same signature for the `A` and `B` portions that matter for binary compatibility.
