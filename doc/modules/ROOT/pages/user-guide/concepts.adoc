...
The `Portable` concept checks if a type is free from platform-dependent members:
...
// Constrain a function to only accept portable types
...
[source,cpp]
----
struct PortablePoint {
    std::int32_t x;
    std::int32_t y;
};

struct NonPortablePoint {
    long x;  // Platform-dependent!
    long y;
};

static_assert(boost::typelayout::Portable<PortablePoint>);
static_assert(!boost::typelayout::Portable<NonPortablePoint>);

NetworkBuffer<PortablePoint> buf1;      // OK
// NetworkBuffer<NonPortablePoint> buf2; // Error: constraint not satisfied
----
...
concept SerializableMessage = 
    boost::typelayout::Portable<T> &&
    std::is_trivially_copyable_v<T> &&
    (sizeof(T) <= 1024);

template<SerializableMessage T>
class MessageQueue {
    // Only accepts portable, trivially copyable types under 1KB
};
----
...
note: because 'boost::typelayout::Portable<BadType>' evaluated to false
note: because 'boost::typelayout::is_portable<BadType>()' evaluated to false
----
...
// Type must be portable and have no bit-fields
template<typename T>
concept CleanLayout = 
    boost::typelayout::Portable<T> &&
    !boost::typelayout::has_bitfields<T>();
...
public:
    template<typename T>
    void write(const T& value) requires boost::typelayout::Portable<T> {
        // Write portable type
    }
    
    template<typename T>
    void write(const T& value) requires (!boost::typelayout::Portable<T>) {
        // Handle non-portable type differently
        static_assert(false, "Cannot serialize non-portable types directly");
    }
};
----
...
* xref:portability.adoc[Portability] - Portability checking details
* xref:hash-verification.adoc[Hash Verification] - Hash-based verification
