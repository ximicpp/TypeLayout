...
// Works equally well with class (private members are reflected)
class SerializableEntity {
public:
    SerializableEntity(std::uint64_t id) : id_(id), active_(true) {}
private:
    std::uint64_t id_;   // Private - still analyzed for serializability
    bool active_;
};
...
static_assert(boost::typelayout::Serializable<SerializableEntity>);  // Classes work too!
static_assert(!boost::typelayout::Serializable<NonSerializablePoint>);

NetworkBuffer<SerializablePoint> buf1;       // OK
NetworkBuffer<SerializableEntity> buf2;      // OK - classes are supported
// NetworkBuffer<NonSerializablePoint> buf3; // Error: constraint not satisfied
----
