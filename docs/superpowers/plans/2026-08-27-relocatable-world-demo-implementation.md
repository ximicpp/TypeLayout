...
`region_handle<T>` is construction-only and may therefore carry a native pointer to its issuing `RegionBuilder`, a checked offset, and a private issuing generation. It is never part of the payload or an Agreement key. All private descriptor writes, including calls to `relative_ptr<T>::reset_unchecked()`, are reached only through active, handle-addressed `RegionBuilder` `bind`/`assign` operations: vector, map, and application-link `bind` use checked construction handles, the party form uses an array handle plus index, and string `assign` allocates character storage from its text before resetting the internal `relative_ptr<char>`; no mutable destination reference leaves the builder. Application `relative_ptr<T>` binding accepts a default null source or requires a non-null source handle to match both the current owner address and generation. A stack destination, a stale or cross-builder handle, or any bind/assign after `finish()` throws `std::invalid_argument` or `std::logic_error` without adding a base address to the stored four-byte representation.
