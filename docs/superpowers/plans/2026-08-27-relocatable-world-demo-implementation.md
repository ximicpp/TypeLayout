...
`region_handle<T>` is construction-only and may therefore carry a native pointer to its issuing `RegionBuilder`, a checked offset, and a private issuing generation. It is never part of the payload or an Agreement key. Only dedicated destination-handle/pointer-to-member `RegionBuilder::bind` overloads, plus the party-array handle/index overload, call private `relative_ptr<T>::reset_unchecked()`; no mutable destination reference leaves the builder. They require the builder to remain active, validate destination provenance and index before typed access, and require every non-null source handle to match both the current owner address and generation. A stack destination, a stale or cross-builder handle, or any bind after `finish()` throws `std::invalid_argument` or `std::logic_error` without adding a base address to the stored four-byte representation.
