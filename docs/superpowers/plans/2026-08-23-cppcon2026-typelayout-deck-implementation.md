...
**Goal:** Produce a fully editable, source-template-faithful 59-slide CppCon 2026 deck that implements the approved signature-centered argument and passes slide-by-slide visual, structural, and technical review.

**Architecture:** Treat the user-provided PPTX as the only visual design source. Inspect all 55 source slides, create a validated 59-slide clone map, edit inherited elements through `@oai/artifact-tool`, and build the deck in five independently rendered content batches before a final evidence and fidelity gate. Keep every intermediate in an isolated build directory and export one new PPTX without overwriting the source attachment.
...
- Main deck: slides 1–43. Appendix: slides 44–59.
...
- Use the terms Evidence, Admission, Agreement, and Permit exactly as defined in the spec.
...
- Color meaning must also be written as `PASS`, `FAIL`, `MATCH`, `DIFFER`, `PERMIT`, or `REJECT`.
...
| `.codex_tmp/cppcon2026_typelayout_final/template-frame-map.json` | Validated mapping from all 59 output slides to source slides and source elements |
...
| `.codex_tmp/cppcon2026_typelayout_final/template-starter.pptx` | 59-slide deck created only by duplicating mapped source slides |
...
| `.codex_tmp/cppcon2026_typelayout_final/slide-content.mjs` | Audience-facing copy, source-slide mapping, edit operations, and note sources for slides 1–59 |
...
### Task 2: Create and validate the 59-slide clone map and starter deck
...
- Produces: a validated 59-slide starter and exact source/starter element targets for every allowed edit.
...
35:28, 36:29, 37:30, 38:31, 39:32, 40:34, 41:40, 42:35, 43:36,
44:37, 45:38, 46:39, 47:40, 48:41, 49:42, 50:44, 51:43, 52:44, 53:48, 54:45,
55:46, 56:51, 57:53, 58:54, 59:55
```

Expected: 59 unique output numbers, all source numbers in the range 1–55.
...
Expected: 59-slide starter, 59 previews, 59 layout JSON files, and a contact sheet.
...
Inspect all 59 starter renders. Confirm order, source-slide correspondence, masters/layouts, CppCon brand marks, typography, footer, page marker, and inherited placeholder state. Record only intentional deviations in `deviation-log.txt`.
...
Add slide 1's definition, run the builder with `--through-slide 1`, export a one-slide-edited/58-slide-preserved draft, and confirm slide 1 renders while slide 2 remains an untouched clone. This smoke build is discarded after validation.
...
- Consumes: the approved spec sections for slides 1–11 and the authoring harness.
...
Use the source mapping from Task 2 and the visible content/visual job from spec section 5.
...
Slide 9 uses the exact chain from spec section 5. It must not be titled or styled as an agenda.
...
Check title wrapping, profile wording, axis labels, connector order, `PERMIT?` suspense, and transition into signature evidence. Record `PASS` or a concrete correction for slides 1–11 in `qa-ledger.txt`; apply corrections before Task 5.
...
- Consumes: repository signature implementation and reflection source notes.
...
Slide 12 must name coverage, canonicality, discrimination, and fail-closed behavior. Slide 13 must state that reflection supplies the byte map and TypeLayout does not infer layout from declaration order.

- [ ] **Step 4: Implement the reflection and recursion sequence**

Slide 14 shows only `nonstatic_data_members_of`, `type_of`, `offset_of`, `bases_of`, and `bit_size_of`, followed by enumerate → recover type → read position → classify → recurse. Slide 15 shows leaf, enum, array, record, union, opaque, and unsupported categories with explicit actions.
...
- [ ] **Step 6: Implement difficult-case and final-certificate slides**

Slide 18 covers arrays, enums, unions, bit-fields, embedded empty objects, opaque contracts, and rejected virtual inheritance. Slide 19 reveals one full `PacketHeader` signature and decodes envelope, record header, offsets, tokens, size, and alignment.

- [ ] **Step 7: Define Agreement precisely**

Slide 20 uses the exact predicate from the spec. Slide 21 shows one exact match and one divergence with readable locations. Slide 22 states three limits: source identity, application meaning, and source-context independence.

- [ ] **Step 8: Add reflection and signature sources to notes**

At minimum cite P2996R12, P3687R1, `include/boost/typelayout/detail/reflect.hpp`, `include/boost/typelayout/detail/signature_impl.hpp`, `include/boost/typelayout/detail/type_map.hpp`, `include/boost/typelayout/signature.hpp`, and `test/test_core.cpp` on the slides whose claims they support.

- [ ] **Step 9: Validate, build, and inspect through slide 22**

Run the validator and builder through slide 22. Inspect slides 12–22 individually. Confirm the signature chapter is visually varied but uses stable token/offset colors, never repeats a full signature without a new inference, and has no invented API.
...
- Consumes: Admission API, signature artifact format, compatibility reporter, and finite-contract model.
- Produces: a profile-aware node predicate, independent two-gate decision model, finite build graph, and provenance-bound CI closure.
...
Slide 23 ends `Agreement PASS / Admission FAIL`. Slide 24 states that pointer rejection follows from the talk's strict source-address-independent profile, not every possible boundary profile.

- [ ] **Step 4: Implement the Admission predicate and ordinary-copy composition**

Slide 25 uses `LocalCopyLegal ∧ NoDetectedContextDependency ∧ EvidenceClosed`. Slide 26 shows `std::is_trivially_copyable_v<T> && is_byte_copy_safe_v<T>` and explicitly states that `is_byte_copy_safe_v<T>` alone is wider than the main permit.

- [ ] **Step 5: Show the structural/semantic limit and complete 2×2 gate**

Slide 27 uses integer-disguised handles. Slide 28 permits only Admission pass plus Agreement match and uses `RepresentationPermit = Admission on every node ∧ Agreement on every required edge`.

- [ ] **Step 6: Define the finite CI contract and per-build emission**

Slide 29 defines `C = (R,V,E,P)`. Slide 30 shows Linux GCC, Linux Clang P2996, and macOS Clang P2996 each producing its own evidence; no node infers another node's layout.

- [ ] **Step 7: Separate artifact fields from provenance**

Slide 31 shows the actual `TypeEntry` fields and platform metadata, while visibly separating source revision, compiler identity, flags, freshness, and workflow identity. Slide 32 puts those producer facts in a CI attestation envelope rather than falsely adding them to `.sig.hpp`.

- [ ] **Step 8: Close the graph without inventing an API**

Slide 33 uses the connected equality graph and actual node-local Admission state. Slide 34 uses current primitive composition or `CompatReporter`; it must not show `TYPELAYOUT_ASSERT_TRANSFER_SAFE`.
...
Confirm Agreement uses edge color/text, Admission uses node color/text, and the final Permit is never inferred from a single macro or skipped CI job. Correct all slide-scoped QA findings before Task 7.

### Task 7: Author and review slides 35–43 — apply both gates and close the talk

**Files:**

- Modify: `.codex_tmp/cppcon2026_typelayout_final/slide-content.mjs`
- Modify: `.codex_tmp/cppcon2026_typelayout_final/edit-plan.json`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-43.pptx`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-43-render/`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-43-layout/`

**Interfaces:**

- Consumes: the two-gate model and recorded platform representation evidence.
- Produces: one permitted type, two independently rejected types, a resolved opening question, runtime obligations, and the final operating rule.

- [ ] **Step 1: Run the partial validator before adding the batch**

Run `validate-content.mjs 43`.

Expected: failure naming slides 35–43 as missing.
...
- [ ] **Step 3: Implement the three-type prediction and first two results**

Use `PacketHeader`, `UnsafeWithPointer`, and `Measurement`. Slide 36 must show `PacketHeader: Admission PASS / Agreement MATCH / PERMIT` and `UnsafeWithPointer: Admission FAIL / Agreement MATCH / REJECT`.

- [ ] **Step 4: Implement the two `Measurement` representations**

Slide 37 shows Linux x86-64: `id @0:u64`, `value @16:fld80[s:16,a:16]`, record size 32/alignment 16. Slide 38 shows Apple ARM64: `id @0:u64`, `value @8:fld64[s:8,a:8]`, record size 16/alignment 8. Keep the shared `[64-le]` prefix visible.

- [ ] **Step 5: Label `Measurement` evidence honestly**

Because `Measurement` is not in the current always-on green repository pipeline, call the results `recorded reference artifacts` in speaker notes. Cite the x86-64 psABI, Apple's ARM64 ABI documentation, and the source-deck/run provenance. Do not state that the default CI currently regenerates both results.

- [ ] **Step 6: Resolve the decision model and operating boundary**

Slide 39 contains the three-row final matrix. Slide 40 separates what TypeLayout proves from application-owned obligations. Slide 41 lists boundary-specific runtime obligations. Slide 42 gives the native-bytes versus serialization decision. Slide 43 repeats the `Measurement` rejection and ends with the exact operating rule from the spec.

- [ ] **Step 7: Validate, build, and inspect through slide 43**

Confirm slide 43 resolves slide 2, contains the GitHub URL/Q&A cue in the inherited footer, and does not become a generic thank-you slide.

### Task 8: Author and review appendix slides 44–59 and complete speaker-note sources

**Files:**

- Modify: `.codex_tmp/cppcon2026_typelayout_final/slide-content.mjs`
- Modify: `.codex_tmp/cppcon2026_typelayout_final/edit-plan.json`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-59.pptx`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-59-render/`
- Create: `.codex_tmp/cppcon2026_typelayout_final/draft-59-layout/`
...
Run `validate-content.mjs 59`.

Expected: failure naming slides 44–59 as missing.
...
44. `Q&A map`
45. `` Why not `has_unique_object_representations`? ``
46. `Padding locations versus padding contents`
47. `Implicit lifetime, storage, alignment, overlap, and synchronization`
48. `Endianness and why byte swapping is conversion`
49. `` Why the global `[64-le]` envelope is conservative ``
50. `` `char`, `bool`, `wchar_t`, and floating-point assumptions ``
51. `Opaque type trust boundary`
52. `Full supported / assumed / rejected matrix`
53. `Full signature grammar and recursive engine pseudocode`
54. `Complete difficult-case encodings`
55. `Diagnostic report anatomy`
56. `Artifact format versus CI provenance`
57. `` Full `TelemetryFrame` signature ``
58. `Ordinary copy versus relocation`
59. `C++29 relocation status and project-policy limits`
...
Slides 45–50 cite the current draft/N5032 clauses they interpret. Slides 58–59 cite P4197R0 and clearly state that trivial relocation was removed from C++26 and remains an open C++29 design space as of 2026-08-23.

- [ ] **Step 5: Add local implementation sources**

Slides 51–57 cite the exact repository files used: opaque registration, signature grammar, parser, safety classification, signature artifacts, and reporter implementation.

- [ ] **Step 6: Validate, build, and inspect all 59 slides**

Run the validator and builder through slide 59. Inspect slides 44–59 individually and verify appendix navigation is clear, titles do not wrap, code remains readable, and the visual treatment is recognizably appendix material without switching themes.
...
- Consumes: the complete 59-slide draft and all claim-supporting implementation/standards sources.
...
- relocation appears only in appendix slides 58–59 or an explicit exclusion statement;
- every required sourced slide contains a `[Sources]` notes block;
- slide 2 contains `PERMIT?`, slides 37–39 answer it, and slide 43 resolves it;
- slides 20, 24, 28, and 40 keep Agreement, Admission, Permit, and application obligations distinct.

- [ ] **Step 2: Run the inspector against the 59-slide draft**

Run:

```powershell
& $env:RUNTIME_NODE 'E:\workspace\TypeLayout\.codex_tmp\cppcon2026_typelayout_final\qa-content.mjs' `
  'E:\workspace\TypeLayout\.codex_tmp\cppcon2026_typelayout_final\draft-59.pptx'
```
...
Check ordinary copy, opaque handling, pointer-like rejection, `TypeEntry`, key-based runtime reporting, and array-position compile-time comparison against the listed API files. Confirm the deck never upgrades current behavior into a nonexistent convenience API.
...
- recorded repository artifacts: `example/sigs/x86_64_linux_clang.sig.hpp` and `example/sigs/arm64_macos_clang.sig.hpp`.

Verify that visible `long double` size/alignment/token claims agree with these sources.
...
For slides 1–59, record `local`, `external`, `conceptual`, or `none required`, with the exact notes block checked for every `local` or `external` row.

### Task 10: Perform final artifact export, 59-slide visual QA, fidelity checks, and delivery copy
...
Run the full builder with `--through-slide 59`, outputting `final-candidate.pptx`, `final-render`, and `final-layout`. Do not promote `draft-59.pptx` directly.
...
- [ ] **Step 6: Inspect all 59 final slides individually at full size**
...
Expected: all commands exit 0, 59 slides are present, and the delivered copy matches the verified candidate.
...
- frame-map validation passed for 59 mapped outputs;
- content validation passed for slides 1–59;
- final artifact contains exactly 59 slides;
- overflow detection passed;
- template-fidelity and structural-placeholder checks passed;
- all 59 QA-ledger rows are fully `PASS`;
