...
**现状**: 签名包含 `[64-le]` 架构前缀
**问题**: 
- 同一个类型（如 `struct { int32_t x; }`）在 32 位和 64 位系统上**应该兼容**（布局相同）
- 但当前设计会生成不同的签名，导致误报不兼容
- 用户主要关心的是**数据布局**，而非指针大小

**建议**: 考虑分离架构信息，提供 `get_data_layout_signature<T>()` 返回不含架构前缀的签名
