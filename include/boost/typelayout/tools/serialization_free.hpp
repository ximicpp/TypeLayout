...
//
// Serialization-free requires three conditions:
//   (1) trivially_copyable(T)           -- memcpy preserves object model
//   (2) !has_pointer(T)                 -- no address-space dependencies
//   (3) signature_match(local, remote)  -- identical layout on both ends
//
...
// is_transfer_safe<T>(remote_sig) -- all three conditions in one call.
// Conditions (1)+(2) compile-time, condition (3) runtime.
template <typename T>
